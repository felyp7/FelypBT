const { UserstateMessage } = require('dank-twitch-irc');
...
const client = new tmi.Client({
	...
	identity: {
		username: 'juicerb0t',
		password: 'oauth:3a4mvr6nsu3fby85frhmpmn6h8awnv'
	},
...
client.on("message", (channel, user, message, self) => {
 ...
 const size = args[1] 
    ...
    if(message.toLowerCase() === "'kek") {
        if (!block) {
            console.log(client.say(channel, `TriHard`));
            block = true;
            setTimeout(() => {
                block = false;
            }, (5 * 1000));
    ...
    if (message.toLowerCase().startsWith("'pyramid") && command === 'pyramid') {
        if(!block) {     
            if(size > 10) {client.say(channel, 'the maximum size is 10' )
            ;return;}
    ...
    if(message == "'ping") {    
            ...
            console.log(data);
            client.say(channel, `FeelsDankMan 🏓 Pong! Latency is ${Math.floor(Math.round(data*1000))}ms | RAM: ${Math.round(process.memoryUsage().rss / 1024 / 1024)}mb`)
            block = true;
            setTimeout(() => {
                block=false;
            }, (5 * 1000));
    ...
    if(message.toLowerCase().startsWith("'7tvsearch") && command === '7tvsearch') {
        if(!block) {
            if(args.join(" ") == []) {client.say(channel, 'No emote provided' )
            ;return;}
            block = true;
            setTimeout(() => {
                block=false;
    ...
    if(message.toLowerCase().startsWith("'emotes") && command === 'emotes') {
        if(!block) {
            if(args.join(" ") == []) {client.say(channel, 'No channel provided' )
            ;return;}
            client.color("Chocolate")
            client.say(channel, `@${user.username} https://e.wrnv.xyz/list/${args.join(" ")} https://emotes.raccatta.cc/twitch/${args.join(" ")}`)
            block = true;
            setTimeout(() => {
                block=false;
            }, (5 * 1000));
    ...
    if(message.toLowerCase().startsWith("'logs") && command === 'logs') {
        if(!block) {
            if(args.join(" ") == []) {client.say(channel, `No channel provided, usage: 'logs (channel)`)
            ;return;}
            client.color("Red")
            client.say(channel,`@${user.username} https://logs.apulxd.ga/?channel=${args.join(" ")}&username=${user.username} or https://logs.ivr.fi/?channel=${args.join(" ")}&username=${user.username}`)
            block = true;
            setTimeout(() => {
                block=false;
            }, (5 * 1000));
